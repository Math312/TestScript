...
def score_argument(pred_argument, truth_argument, others):
    pred_event = others[0]
    truth_event = others[1]
    arg_id_to_coref = others[2]["arg_id_to_coref"]
    pred_trigger_id = score_util.get_trigger_unique_id(pred_event["event_type"], pred_event["trigger"])
    truth_trigger_id = score_util.get_trigger_unique_id(truth_event["event_type"], truth_event["trigger"])
    if pred_trigger_id != truth_trigger_id:
        return 0
    else:
        pred_arg_id = score_util.get_argument_unique_id(pred_argument)
        truth_arg_id = score_util.get_argument_unique_id(truth_argument)
        if pred_argument["role"] == truth_argument["role"] and arg_id_to_coref[pred_arg_id] == arg_id_to_coref[
            truth_arg_id]:
            return -1
        else:
            return 0
...
def score_argument_without_trigger(pred_argument, truth_argument, others):
    pred_event = others[0]
    truth_event = others[1]
    arg_id_to_coref = others[2]["arg_id_to_coref"]
    pred_arg_id = score_util.get_argument_unique_id(pred_argument)
...
def post_process_func(pred_list, truth_list, score_func, score_matrix, row_idx, col_idx):
    row_result = []
    col_result = []
    score = 0
    for idx in range(0, len(row_idx)):
        if score_matrix[row_idx[idx]][col_idx[idx]] != 0:
            ...
            score += -score_matrix[row_idx[idx]][col_idx[idx]]
    return row_result, col_result, score


def score_full_event(pred_event, truth_event, others):
    pred_arguments = pred_event["arguments"]
    truth_arguments = truth_event["arguments"]
    row_idx, col_idx, max_score = score_util.hungarian(pred_arguments, truth_arguments, score_argument,
                                                       others=[pred_event, truth_event, others],
                                                       post_process_func=post_process_func)
    result = -len(row_idx)
    return result


def score_event_without_trigger(pred_event, truth_event, others):
    pred_arguments = pred_event["arguments"]
    truth_arguments = truth_event["arguments"]
    row_idx, col_idx, max_score = score_util.hungarian(pred_arguments, truth_arguments, score_argument_without_trigger,
                                                       others=[pred_event, truth_event, others],
                                                       post_process_func=post_process_func)
    result = -len(row_idx)
    return result
...
def score(pred_file, truth_file, ontology):
    with open(pred_file, "r", encoding="utf-8") as pred_fd, open(truth_file, "r", encoding="utf-8") as truth_fd, open(
        ...
        sum_correct_argument_without_trigger_count = 0
        match_sequence_json = []
        for (pred_document, truth_document) in zip(pred_content, truth_content):
            ...
            full_row_idx, full_col_idx, full_max_score = score_util.hungarian(pred_event_list, truth_event_list,
                                                                              score_func=score_full_event,
                                                                              others={
                                                                                  "coref_dict": coref_dict,
                                                                                  "arg_id_to_coref": arg_id_to_coref
                                                                              }, post_process_func=post_process_func)
            without_trigger_row_idx, without_trigger_col_idx, without_trigger_max_score = score_util.hungarian(
                pred_event_list, truth_event_list,
                score_func=score_event_without_trigger,
                others={
                    "coref_dict": coref_dict,
                    "arg_id_to_coref": arg_id_to_coref
                }, post_process_func=post_process_func)
            ...
            document_truth_sequence = []
            for idx in range(0, len(full_row_idx)):
                document_truth_sequence.append([int(full_col_idx[idx]), int(full_row_idx[idx])])
            ...
            sum_correct_argument_count += full_max_score
            sum_correct_argument_without_trigger_count += without_trigger_max_score
        sequence_writer.write(json.dumps(match_sequence_json))
        full_p, full_r, full_f1 = score_util.get_p_r_f1(sum_correct_argument_count, sum_pred_argument_count,
                                                        sum_truth_argument_count)
        without_trigger_p, without_trigger_r, without_trigger_f1 = score_util.get_p_r_f1(
            sum_correct_argument_without_trigger_count, sum_pred_argument_count,
            sum_truth_argument_count)
        logging.info(
            "Full Arguments(Contain [Event Type][Trigger][Arguments]):P:{},R:{},F1:{}".format(full_p, full_r, full_f1))
        logging.info(
            "Without Trigger And Event Type(Contain [Arguments]):P:{},R:{},F1:{}".format(without_trigger_p,
                                                                                         without_trigger_r,
                                                                                         without_trigger_f1))
